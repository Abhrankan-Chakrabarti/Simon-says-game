...
pygame.mixer.music.load("nostalgic.ogg")  # Ensure this file exists in the same folder
...
async def flash(index):
    draw_buttons(highlight=index)
    await asyncio.sleep(0.5)
    draw_buttons()
    await asyncio.sleep(0.25)

async def show_message(text, delay=1000):
    theme = THEMES[theme_names[current_theme_index]]
...
async def wait_for_start(message="Click to Start"):
    global current_theme_index
    draw_buttons()
    theme = THEMES[theme_names[current_theme_index]]
...
async def main():
    while True:
        await wait_for_start("Click to Start")
        running = True
        sequence = []
        level = 1
        ...
        while running:
            await show_message(f"Level {level}")
            ...
            if level in [5, 10, 20]:
                ...
                await show_message(messages[level], delay=1500)
            ...
            for idx in sequence:
                ...
                if user_input != idx:
                    final_score = level - 1
                    await show_message("Game Over!", delay=1000)
                    await show_message(f"Level Reached: {final_score}", delay=1000)

                    if final_score > high_score:
                        save_high_score(final_score)
                        await show_message("🎉 New High Score!", delay=1000)
                        await show_message(f"High Score: {final_score}", delay=1500)
                    else:
                        await show_message(f"High Score: {high_score}", delay=1500)

                    await wait_for_start("Press R to Restart or Q to Quit")
                    waiting = True
                    while waiting:
                        for event in pygame.event.get():
                            if event.type == pygame.QUIT:
